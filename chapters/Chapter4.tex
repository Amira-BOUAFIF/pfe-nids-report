...
\paragraph{Dataset Formatting and Label Corrections :}
\begin{itemize}
...
\vspace{-0.2cm}
\paragraph{Attack Classes description :} 
Getting to understand the target label is just as important as any other step in the data analysis process. In this dataset, the target is particularly challenging because it represents attack classifications. 

Therefore, understanding the nature of each attack is fundamental, as it helps later explain modeling outcomes, interpret misclassifications, and justify preprocessing and modeling decisions. 
...
The data exploration tasks covers the class distribution and feature analysis in order to gain a deeper understanding of the dataset.

\paragraph{Class distribution analysis :}

The class distribution analysis is organized into three aspects: an overall view of the class balance, a per-file breakdown, and a detailed look at the distribution among attack classes.
...
\begin{itemize}
    ...
    The Figure \ref{fig:BENIGN_vs_Attack_ratio} visualizes the BENIGN vs Attack ratio. 
    ...
    The BENIGN class dominates across all files, with the only exceptions being the two Friday PM files, where attack traffic exceeds BENIGN, with attack ratios of 56.7\% and 55.5\% in the Friday PM DDoS and Friday PM PortScan respectively.

    Moreover, the BENIGN ratio ranges from 100\% in the Monday file down to 43.3\% in the Friday PM DDoS file. Several files show very low attack ratios, such as Thursday AM at 1.3\% and Friday AM at 1\%, while the Thursday PM file has an attack ratio so low at 0.012\% that it is invisible in Figure \ref{fig:benign_vs_attack_per_file} that shows the BENIGN vs Attack ratio per file.
    ...
    Each attack class is present in only one file and each file contains between 0 and 5 attack classes, with most files containing only one. The two brute force attacks, FTP-Patator and SSH-Patator are both present in the Tuesday file. In the same way, the DoS attacks and Heartbleed are grouped in the Wednesday file, and the web attacks are present in the Thursday AM file. The remaining classes each appear exclusively in one of the other files. Figure \ref{fig:class_dist_file} visualizes the class distribution per file.
    ...
    In addition, the class imbalance extends beyond the BENIGN class to among the attack classes themselves. DoS Hulk, PortScan, DDoS and DoS GoldenEye each exceed 10,000 samples, providing the model with sufficient examples to reliably learn their patterns. FTP-Patator, SSH-Patator, DoS slowloris, DoS Slowhttptest, Bot and Web Attack - Brute Force fall in a moderate range, each with over 1,000 samples, which while on the lower end, should still allow the model to capture their general patterns.
    ...
    Finally, the three remaining classes represent the most critical imbalance: Infiltration with 36 samples, Web Attack - Sql Injection with 21 samples, and Heartbleed with only 11 samples. 

\end{itemize}

\paragraph{Feature Analysis: }
We begin the feature space exploration by computing the correlation matrix for the 78 features of the dataset. 
It is a square matrix where each element represents the linear association between two features, with values ranging from $-1$ to $1$. 
Positive values indicate that both features increase together, while negative values indicate that one increases as the other decreases. 
In both cases the strength of the correlation increases as the value approaches $1$ or $-1$, while a value of zero indicates no correlation. 
The diagonal is always $1$ as every feature is perfectly correlated with itself.
...
39 highly correlated pairs are identified Figure \ref{fig:correlation_heatmap} visualizes their correlation matrix.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.97\columnwidth, fbox]{figures/correlation_heatmap.png}
...
8 pairs have a perfect correlation of 1, meaning they are perfectly linearly correlated. Table \ref{tab:correlated_features} lists these perfectly correlated pairs.
...
\begin{itemize}
    ...
    \item \textbf{Sixth pair:} \texttt{Fwd PSH Flags} and \texttt{SYN Flag Count} are nominally
          designed to capture different aspects of TCP behavior, with the first counting forward packets
          with the PSH flag set and the second counting all packets with the SYN flag set across the flow.
          However, they are perfectly correlated throughout the dataset, with exactly 2,699,265 flows
          where both equal 0 and 131,478 flows where both equal 1, a result of two compounding causes.

          The first is a bug in the generation-time version of CICFlowMeter where flags are only updated
          on the first packet, reducing both features to binary indicators of whether that single packet
          had the respective flag set. This means the SYN-ACK from the server is never counted,
          contradicting what the feature name implies.

          The second is a traffic capture artifact. Flows where both equal 1 belong exclusively to attacks
          that establish real TCP connections (\texttt{FTP-Patator}, \texttt{SSH-Patator}, \texttt{DoS
              slowloris}, \texttt{DoS Slowhttptest}, \texttt{Infiltration}) and benign traffic, while flows
          where both equal 0 include attacks that bypass the handshake (\texttt{DDoS}, \texttt{DoS Hulk},
          \texttt{DoS GoldenEye}, \texttt{PortScan}, \texttt{Bot}, Web Attacks) and mid-stream captures.
          In this dataset, any flow starting with a SYN packet consistently has PSH set on that same
          first packet, making both features identical binary indicators of whether the flow was captured
          from the beginning of a TCP connection, and therefore entirely redundant.
    \item \textbf{Seventh pair:} \texttt{Fwd URG Flags} and \texttt{CWR Flag Count} are perfectly correlated throughout the dataset, with 2,830,428 flows where both equal 0 and only 315 flows where both equal 1, all belonging exclusively to benign traffic. As with the previous pair, the flag-counting bug reduces both features to binary indicators of the first packet only. The co-occurrence of URG and CWR on the same first packet is consistent with legitimate TCP congestion control behavior, which explains their absence in attack traffic, making both features redundant with each other.
    \item \textbf{Eighth pair:} \texttt{Fwd Header Length} and \texttt{Fwd Header Length.1} these two features are duplicates of each other as mentioned previously in the Data Description subsection \ref{Sec:Desc}, which makes sense for them to be perfectly correlated.
\end{itemize}
...
The class-level distribution of affected rows is presented in Figure
\begin{figure}[H]
    \centering
...
\end{figure}
\vspace{-0.9cm}
\paragraph{Negative Values:} Given the nature of the dataset feature, their logical value should never be negative yet Negative values were found across several feature groups, each with a distinct cause.
\begin{itemize}
...
\vspace{-0.2cm}
\begin{itemize}
    \item \textbf{Packet and Segment length mean pairs:} \texttt{Fwd Packet Length Mean} and \texttt{Avg Fwd Segment Size}, \texttt{Bwd Packet Length Mean} and \texttt{Avg Bwd Segment Size} \item \textbf{Packet and Segment length mean pairs:} \texttt{Fwd Packet Length Mean} and \texttt{Avg Fwd Segment Size}, \texttt{Bwd Packet Length Mean} and \texttt{Avg Bwd Segment Size}, the packets features are kept over the segments one, as they use standard network traffic analysis terminology, are immediately interpretable, and are consistent with the other packet length features already present in the dataset.
    \item \textbf{Totals and Subflows pairs:} \texttt{Total Fwd Packets} and \texttt{Subflow Fwd Packets}, \texttt{Total Backward Packets} and \texttt{Subflow Bwd Packets}, \texttt{Total Length of Fwd Packets} and \texttt{Subflow Fwd Bytes}, and \texttt{Total Length of Bwd Packets} and \texttt{Subflow Bwd Bytes}, as established before, the subflows are equal to the totals throughout the dataset due to a bug in their computation. The totals are kept as they are both correctly computed and more fundamental,representing the actual flow-level counts and byte totals directly. The subflow features are dropped as they provide no additional information beyond what the totals already capture.
    \item \textbf{Flags pair:} \texttt{Fwd PSH Flags} and \texttt{SYN Flag Count}, Although they are both only counted on the first packet, they still provide information about whether the flow was captured from the beginning of a TCP connection. \texttt{SYN Flag Count} is kept as it represents the synchronization packets counts used to initiate TCP connections making it more useful for that purpose, while the \texttt{Fwd PSH Flags} represents the count of how many forward packets had the push (PSH) flag set signaling the sender to push buffered data immediately.
    \item \textbf{Flags pair:} \texttt{Fwd PSH Flags} and \texttt{SYN Flag Count}, although both are only counted on the first packet due to the flag-counting bug, they still provide information about whether the flow was captured from the beginning of a TCP connection. \texttt{SYN Flag Count} is kept as it captures this condition more directly, reflecting whether a connection-initiating SYN packet was observed, which is more meaningful for distinguishing attack types than \texttt{Fwd PSH Flags}, which only indicates whether the first packet had the PSH flag set and carries no additional interpretable information beyond what \texttt{SYN Flag Count} already provides.
