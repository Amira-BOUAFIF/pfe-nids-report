...
\begin{itemize}
    \item \textbf{Feature Description:} The dataset contains 79 columns, of them 78 are numeric level-flow features and the last one is the target Label. These features can be grouped into 12 categories as shown in Table \ref{tab:features}.
          ...
          \begin{itemize}
              ...
              \item \texttt{Packet Length Mean} and \texttt{Average Packet Size} both measure the average packet size, yet the two are equal only when zero. This inequality is caused by a bug in the denominator of \texttt{Packet Length Mean}, as it uses the internal \texttt{flowLengthStats} accumulator to get its value and the first packet is added twice to it, inflating its packet count by one, while \texttt{Average Packet Size} divides by the correctly computed total packet count~\cite{CICFlowMeter}.
          \end{itemize}
\end{itemize}
