...
\begin{itemize}
    ...
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.4\columnwidth, fbox]{figures/benign_attack_ratio.png}
    ...
    \item \textbf{Class distribution per file:} Figure \ref{fig:benign_vs_attack_per_file} shows the BENIGN vs Attack ratio per file. 
    The BENIGN class dominates across all files. However, the only exceptions are the two Friday PM files, where attack traffic exceeds BENIGN, with attack ratios of 56.7\% and 55.5\% in the Friday PM DDoS and Friday PM PortScan respectively.

    Moreover, the BENIGN ratio ranges from 100\% in the Monday file down to 43.3\% in the Friday PM DDoS file. Several files show very low attack ratios, such as Thursday AM at 1.3\% and Friday AM at 1\%, while the Thursday PM file has an attack ratio so low at 0.012\% that it is invisible in the figure.
    ...
    Figure \ref{fig:class_dist_file} visualizes the class distribution per file. As mentioned previously, each attack class is present in only one file.

    Furthermore, each file contains between 0 and 5 attack classes, with most files containing only one. The two brute force attacks, FTP-Patator and SSH-Patator are both present in the Tuesday file. In the same way, the DoS attacks and Heartbleed are grouped in the Wednesday file, and the web attacks are present in the Thursday AM file. The remaining classes each appear exclusively in one of the other files.
    ...
    \item \textbf{Attack classes distribution:} Figure \ref{fig:attack_distribution} visualizes the global attack distribution. It varies significantly, ranging from DoS Hulk at 8.16\% down to Heartbleed at 0.0004\%, with the three rarest classes, Infiltration, Web Attack - Sql Injection, and Heartbleed, collectively accounting for only 0.0024\%.

    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.8\columnwidth, fbox]{figures/Attack_only_Distribution_log.png}
